...
| mssql\_elastic\_pool\_sku\_families | The family of hardware `Gen4` or `Gen5`. | `list(string)` | `[]` | no |
...
| mssql\_elastic\_pool\_sku\_tiers | The tier of the particular SKU. Possible values are `GeneralPurpose`, `BusinessCritical`, `Basic`, `Standard`, or `Premium`. For more information see the documentation for your Elasticpool configuration: `vCore-based` or `DTU-based`.If enabled value is `REQUIRED` | `list(string)` | `[]` | no |
